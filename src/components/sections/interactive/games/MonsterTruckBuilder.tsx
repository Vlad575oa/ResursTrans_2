...
import { Truck, Settings2, Sparkles, Wrench, Palette, Ruler } from "lucide-react";

const PARTS = {
    wheels: [
        { id: "standard", label: "Стандарт", size: "scale-100", color: "bg-slate-700" },
        { id: "offroad", label: "Внедорожные", size: "scale-125", color: "bg-slate-900 border-2 border-slate-600" },
        { id: "monster", label: "Монстр-катки", size: "scale-150", color: "bg-black border-4 border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.5)]" },
    ],
    body: [
        { id: "delivery", label: "Фургон", color: "border-slate-400" },
        { id: "pickup", label: "Пикап", color: "border-emerald-400" },
        { id: "tanker", label: "Танкер", color: "border-blue-400" },
    ],
    size: [
...
export default function MonsterTruckBuilder() {
    ...
    return (
        <div className="relative w-full min-h-[500px] bg-gradient-to-br from-slate-950 via-zinc-900 to-slate-950 rounded-3xl border border-white/10 p-8 overflow-hidden group">
                    ...
                    >
                        <AnimatePresence mode="wait">
                            <motion.div
                                ...
                                className={`w-48 h-24 rounded-t-2xl border-x-8 border-t-8 transition-colors duration-500 ${selections.body.color} ${selections.paint.class} relative`}
                            >
                                {/* Cab */}
                                <div className="absolute -left-12 bottom-0 w-12 h-16 bg-inherit border-l-8 border-t-8 border-slate-900 rounded-tl-2xl overflow-hidden">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-6 py-2 bg-white/5 backdrop-blur-md rounded-full border border-white/10 flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-500">
                <span>MASS: {Math.round(selections.size.scale * 4500)} KG</span>
                <div className="w-[1px] h-3 bg-white/10" />
                <span className="text-amber-500">STABILITY: OK</span>
            </div>
        </div>
    );
}
